...
def dropUnknwonCountries(df, KNOWN_COUNTRIES):
...
def transformToLongFormat(df, metric_name):
    df = pd.melt(df, id_vars=["Country"], value_vars = getDfColumnsToBecomeRows(df, ["country_code","year"]),
                   var_name='year', value_name=metric_name)                  
    return df
...
def cleanEconomics(cleandFilesPath):
    ...
    print("Cleaning: Income by Country.xlsx ")
    for sheet_name in sheet_names:
        ...
        df = dropUnknwonCountries(df, KNOWN_COUNTRIES)
